...
def combine_datasets(*datasets):
    ...
    for i, (images, labels) in enumerate(datasets):
        ...
        # Flatten labels if they are not 1D
        if labels.ndim > 1:
            labels = np.argmax(labels, axis=-1)  # Convert one-hot to single label
...
def main():
    ...
    chestmnist = np.load(os.path.join(DATA_DIR, 'chestmnist.npz'))
    dermanmnist = np.load(os.path.join(DATA_DIR, 'dermamnist.npz'))
    ...
    chestmnist_data = (chestmnist['train_images'], chestmnist['train_labels'].squeeze())
    dermanmnist_data = (dermanmnist['train_images'], dermanmnist['train_labels'].squeeze())
    
    # Combine datasets
    combined_images, combined_labels = combine_datasets(bloodmnist_data, pathmnist_data, chestmnist_data, dermanmnist_data)
