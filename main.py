...
def main():
    ...
    # Split in 3 parts - 65, 25, 10%s - Pretext train/test
    # Run 1 = Model on Pretext dataset 
    # Run 2 --> Model B = Model A finetune.train_dataset 
    # Run 3 -->  Model B2 = ModelC train on 25%
    # Run 4 --> Model B2 = ModelC train on 95% (Need 4 sep datasets)(On the same test sets!!!!)
    ...
    test_loader = DataLoader(MainTaskDataset(*zip(*test_dataset), transform=data_transform), batch_size=BATCH_SIZE, shuffle=False)
    full_train_loader = DataLoader(MainTaskDataset(*zip(*full_train_dataset), transform=data_transform), batch_size=BATCH_SIZE, shuffle=True)
    full_dataset_loader = DataLoader(MainTaskDataset(combined_images, combined_labels, transform=data_transform), batch_size=BATCH_SIZE, shuffle=True)
    ...
    # Training Model B4 on 100% of the data
    model_b4 = MainTaskNet(in_channels=3, num_classes=len(np.unique(combined_labels))).to(DEVICE)
    model_b4 = train_model(model_b4, full_dataset_loader, num_epochs=NUM_EPOCHS, device=DEVICE)

    print('==> Evaluating Model B4 (trained on 100%) on test set...')
    evaluate_model(model_b4, test_loader, DEVICE)
